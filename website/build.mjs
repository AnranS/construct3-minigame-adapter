...
function apiTable() {
  ...
  const rows = catalog.map(row => {
    ...
    const cells = API_PLATFORMS.map(platform => {
      if (!row.kinds[platform]) return '<td><span class="unsupported">未列入</span></td>';
      const contract = row.contracts[platform].map(note => `<small>${escape(note)}</small>`).join('');
      return `<td><span class="kind-badge">${escape(row.kinds[platform])}</span>${contract}</td>`;
    }).join('');
...
for (const [slug,title,description, customSource] of pages) {
  ...
  const body = `<main id="main" class="doc-layout"><aside class="doc-sidebar" aria-label="文档导航"><p class="sidebar-label">使用文档 · v${version}</p><nav>${navLinks}</nav><div class="sidebar-separator"><a href="${base}downloads/C3MiniGameBridge.c3addon" download>下载插件 ↓</a><a href="${base}downloads/MiniGameApiSuite.c3p" download>下载示例工程 ↓</a><a href="${repo}">GitHub ↗</a></div></aside><article class="doc-content"><details class="doc-mobile-nav"><summary>文档目录 · ${title}</summary><nav aria-label="移动端文档导航">${navLinks}</nav></details><p class="eyebrow">CONSTRUCT MINI GAME / ${slug.toUpperCase()}</p>${articleHTML}<div class="doc-bottom"><a href="${repo}/blob/main/${source}">在 GitHub 查看本文 ↗</a><span>v${version} · 2026-09-20</span></div></article><aside class="doc-toc" aria-label="页内目录"><strong>本页内容</strong>${headings.map(h=>`<a href="#${escape(h.id)}">${escape(h.text)}</a>`).join('')}</aside></main>`;
