...
## 2026-09-21 图片 demo 与文档更新

当前 `.c3p` 源工程新增“图片渲染”分类，使用包内 `render-test.png`、`runtime.assets.fetchBlob` 和真正的 Construct Sprite。图片加载、等比排版、隐藏、分类返回和异步失败已有回归覆盖；本机完整 `npm test` 为 **324 项通过、0 失败、0 跳过**，插件与三端 smoke 构建通过。

本次源工程尚未完成重新导出及小游戏 IDE / 真机图片验收。旧 HTML5 导出和旧小游戏包不包含图片分类；下面的导出哈希、53 / 57 入口及 IDE 画面记录均为更新前证据。当前源码示例为微信 / 抖音 **54 项、13 类**，TikTok **58 项、14 类**。生成素材不是平台运行截图。
